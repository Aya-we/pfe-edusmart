...
export default function StudentSchedulePage() {
  ...
  const pdfUrl = `${API}/timetable-upload/students`;

  const check = async () => {
    setStatus("loading");
    try {
      const res = await axios.get(`${API}/timetable-upload/check/students`);
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
          ...
          <Card className="rounded-2xl border border-border bg-background overflow-hidden shadow-sm">
            <CardContent className="p-0">
              <iframe src={`${pdfUrl}#toolbar=0`} className="w-full" style={{ height: "75vh", minHeight: 500 }} title="Emploi du temps" />
            </CardContent>
          </Card>
        </motion.div>
      )}
